...
import { UserService } from '@yr/user/user.service';
...
export class FavoriteComponent {
  ...
  ngOnInit() {
    const isFavorite =
      ...
      this.authQuery.getUserInfo().favorites.includes(this.productId);
  ...
  toggleActive() {
    this.authDialogService.verifyLoggedIn(() => {
      ...
      this.updateHeartState();
      (this.active
        ? this.userService.addToWatchlist(this.productId)
        : this.userService.removeFromWatchlist(this.productId)
      ).subscribe();
    });
  }
