...
import { toBase64 } from '@yr/shared/utils';
...
export class MarketplaceAddItemComponent implements OnInit, OnDestroy {
  ...
  productForm = this.fb.group({
    image: [
      null,
      [Validators.required, allowedFileTypes(this.ALLOWED_FORMATS)]
    ],
  ...
  submit() {
    ...
    if (
      ...
      this.imageBase64 !== this.originalProduct.image
    ) {
      product.image = this.imageBase64;
    }
    const serverAction = this.originalProduct
      ? this.marketplaceService.updateProduct(this.originalProduct._id, product)
      : this.marketplaceService.addProduct(product);
    serverAction
      .pipe(
        finalize(() => {
          this.loading = false;
      ...
      .subscribe(() => {
        this.snackbar.success(
          this.originalProduct
            ? 'edit-item.successfully-updated'
  ...
  private initData() {
    const serverRequests: Observable<any>[] = [
      this.marketplaceService.getAllCategories()
    ];
    ...
    combineLatest(serverRequests).subscribe(
      ([categories, product]: [Category[], Product?]) => {
        ...
        if (product) {
          ...
          const selectedSubCategory = this.subCategoriesMap[
            selectedCategory.name
          ].find(({ name }) => name === subCategory.name);
